...
if __name__ == "__main__":
    ...
    plt.scatter([x.length for x in clusters[0].points], [x.tracks for x in clusters[0].points], c='r')
    plt.scatter([x.length for x in clusters[1].points], [x.tracks for x in clusters[1].points], c='g')
    # plt.scatter([x.length for x in clusters[2].points], [x.tracks for x in clusters[2].points], c='b')
    plt.xlabel("Length")
    plt.ylabel("Tracks")
    plt.show()
